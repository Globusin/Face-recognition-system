...
def start_hotspot(ssid, password):
    ...
    """
    try:
        log_debug(f"Попытка создания Wi-Fi хотспота с SSID: {ssid}")
        
        # Останавливаем возможную предыдущую точку доступа
        log_debug("Остановка предыдущей точки доступа (если существует)")
        ...
        log_debug("Создание новой точки доступа")
